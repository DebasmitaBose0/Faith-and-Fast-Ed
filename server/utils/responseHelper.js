export const mapMessageToErrorCode = (message) => {
  ...
  if (msg.includes('not authorized') || msg.includes('not admin'))
    return 'NOT_AUTHORIZED';
  if (msg.includes('product not found') || msg.includes('product is not found'))
    return 'PRODUCT_NOT_FOUND';
  if (msg.includes('invalid otp') || msg.includes('incorrect otp'))
    return 'INVALID_OTP';
  if (msg.includes('otp expired') || msg.includes('otp has expired'))
    return 'OTP_EXPIRED';
  if (
    msg.includes('required fields') ||
    msg.includes('validation failed') ||
    msg.includes('please enter all')
  )
